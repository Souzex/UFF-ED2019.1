...
int main() {
    ...
    while(1) {
        ...
        scanf("%s",cadeia); 
        printf("Agora informe um número 'n' de primeiros caracteres a serem retirados da cadeia: ");
        scanf("%d",&num); 

        for(i=0; cadeia[i++]; tam=i); // for(i=0; cadeia[i]; tam=++i);
        
        retira_inicio_n (cadeia, num); // retira_inicio_n (&cadeia[0], num);
...
void retira_inicio_n (char *str, int n) {
    ...
    int i;
    // for(i=0; str[n+i]; str[i] = str[n+(i++)]);
    // for(i=0; str[n+i]; str[i] = str[n+i++]);
    for(i=0; str[n+i]; str[i] = str[(i++)+n]); 
    str[i] = '\0';

    // for(i=0; str[i]; i<n ? i++ : (str[i-n] = str[i++]));
    // str[i-n] = '\0';
}